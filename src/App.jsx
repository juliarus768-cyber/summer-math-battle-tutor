...
export default function App() {
  ...
  const toggleWeekend = () => save({ ...state, weekendMode: !state.weekendMode });
  ...
  return (
    <div className="app-shell">
      ...
      <header className="hud">
        <div className="logo"><span className="logo-mark">⚔️</span><span>Summer Math<br /><b>Battle Tutor</b></span></div>
        <nav aria-label="Main navigation">
          {tabs.map((t) => <button key={t} className={state.activeTab === t ? 'tab active' : 'tab'} onClick={() => switchTab(t)}>{tabIcon(t)} {t}</button>)}
        </nav>
      ...
      <main className="content">
        ...
        {!['Home', 'Store', 'Parent'].includes(state.activeTab) && <Module title={state.activeTab} state={state} toggleWeekend={toggleWeekend} />}
      </main>
    </div>
  );
}
...
function tabIcon(tab) {
  return ({ Home: '🏠', Missions: '🎯', Battle: '⚔️', Practice: '🧠', 'Money Lab': '💰', Store: '🛒', 'Grant Prize': '🎁', Parent: '📊' })[tab] || '⭐';
}

function Home({ state, completeMission, claimGrant }) {
  return <section className="fade-in">
    <div className="event-banner"><div><h2>DOUBLE XP WEEKEND</h2><p>Portal boost expires in 01d 12h 33m</p></div><strong>2X</strong></div>
    ...
    <div className="status-grid">
      ...
      <Card title="Team Bonus" value="Both finish = +40 coins" icon="🛡️" />
    </div>
    <div className="section-title">Today’s Missions</div>
    <div className="mission-grid">
      {missions.map((m, i) => <article key={m.name} className={`mission ${m.rarity}`}><h4>{m.icon} {m.name}</h4><p>{m.desc}</p><div className="mini-bar"><span style={{ width: `${(state.missionProgress[i] / 5) * 100}%` }} /></div><div>{state.missionProgress[i]}/5 • +{m.xp} XP</div><div className="check">{state.missionProgress[i] >= 5 ? '✅ Complete' : '⬜ In progress'}</div></article>)}
    </div>
    ...
    <div className="lower-grid">
      ...
      <article className="big-card"><h3>Next Reward</h3><p>🧋 Bubble Tea unlock: 80 coins</p></article>
      ...
      <article className="big-card motivation"><h3>Katya Motivation</h3><p>“Friends don’t lie. Numbers don’t either. Keep solving.”</p></article>
    </div>
  </section>;
}

function HeroCard({ kid, data, theme }) {
  const isKatya = kid === 'katya';
  return <article className={`hero ${theme}`}>
    <div className={`avatar ${isKatya ? 'katya-avatar' : 'alex-avatar'}`}>
      <div className="avatar-glow" />
      <div className="hair" />
      <div className="face"><span className="eyes">● ●</span><span className="smile">{isKatya ? '▱▱▱' : ')'}</span></div>
      <div className="outfit">{isKatya ? '🔦' : '⚡'}</div>
    </div>
    <div className="hero-info"><h3>{isKatya ? 'Katya: Mystery Detective' : 'Alex: Battle Commander'}</h3><p>Level {data.level} • {data.rank}</p><div className="xp"><span style={{ width: `${(data.xp / data.xpMax) * 100}%` }} /></div><p>🪙 {data.coins} • 🔥 {data.streak} • Bonus: {data.bonuses[0]}</p><button>View Profile</button></div>
  </article>;
}

function Card({ title, value, icon }) { return <article className="card"><h4>{icon} {title}</h4><p>{value}</p></article>; }
...
function Parent({ state }) { return <section className="fade-in"><h2>Parent Dashboard</h2><div className="parent-grid"><article><h3>Daily Minutes</h3><p>Alex: {state.players.alex.minutes} • Katya: {state.players.katya.minutes}</p></article><article><h3>Missions Completed</h3><p>Alex: {state.players.alex.missions} • Katya: {state.players.katya.missions}</p></article><article><h3>Weak Skills</h3><p>Alex: fractions, long division | Katya: subtraction fluency, division facts</p></article><article><h3>Strongest Skill Today</h3><p>Alex: BEDMAS checks | Katya: persistence in clue-solving</p></article><article><h3>Suggested Focus</h3><p>6 x 10 minutes each. Reward corrections and effort.</p></article><article><h3>Phrase to use</h3><p>“Good try. Let’s fix the step.”</p></article></div><h3>Reward Requests</h3><ul className="requests">{state.rewardRequests.length ? state.rewardRequests.map((r, idx) => <li key={idx}>{r.time} — {r.kid} requested {r.rewardName} ({r.cost} coins) [{r.status}]</li>) : <li>No requests yet.</li>}</ul></section>; }

function Module({ title, state, toggleWeekend }) {
  return <section className="fade-in module"><h2>{title}</h2><p>This module is ready for Phase 2 logic. The visual shell is active now.</p><button onClick={toggleWeekend}>{state.weekendMode ? 'Switch to weekday mode' : 'Toggle weekend mode'}</button><p>{state.weekendMode ? 'Weekend mode: lighter 8-minute sessions.' : 'Weekday mode: standard 10-minute sessions.'}</p></section>;
}
